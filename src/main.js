...
app.whenReady().then(async () => {
  ...
  activeWindowProvider = new ActiveWindowProvider({
    ...
    helperRoot: app.isPackaged ? path.join(process.resourcesPath, 'native') : path.join(packageRoot, 'build', 'native')
  });
...
function createOverlayWindow() {
  ...
  ipcMain.on('overlay:measured', (_event, measured) => {
  ...
  const template = [
    ...
    },
    {
      label: telemetry.configured
        ? (isChinese() ? '匿名每日心跳' : 'Anonymous daily heartbeat')
        : (isChinese() ? '匿名统计（当前版本未配置）' : 'Anonymous analytics (not configured)'),
      type: 'checkbox',
    ...
    {type: 'separator'},
    menuItem(isChinese() ? '复制简短诊断信息' : 'Copy short diagnostics', () => clipboard.writeText(diagnostics.short())),
    menuItem(isChinese() ? '隐私说明' : 'Privacy', () => shell.openExternal('https://github.com/cpys/codex-quota-overlay/blob/main/PRIVACY.zh-CN.md')),
    menuItem(isChinese() ? '检查更新' : 'Check for updates', () => shell.openExternal('https://github.com/cpys/codex-quota-overlay/releases')),
    menuItem(isChinese() ? '关于' : 'About', () => dialog.showMessageBox({
      type: 'info',
...
function sendOverlayState(state) {
  ...
  if (overlayWindow.webContents.isLoading()) {
    overlayWindow.webContents.once('did-finish-load', () => overlayWindow?.webContents.send('overlay:state', payload));
  } else {
...
function normalizeBounds(bounds) {
  ...
  if (process.platform === 'win32' && typeof screen.screenToDipPoint === 'function') {
    ...
    const bottomRight = screen.screenToDipPoint({x: Math.round(bounds.x + bounds.width), y: Math.round(bounds.y + bounds.height)});
...
function adjustPlacement(xDelta, yDelta) {
  ...
  settings.update({placement: {x: clamp(current.x + xDelta, -80, 80), y: clamp(current.y + yDelta, -40, 40)}});
  if (currentTargetBounds) positionOverlay(currentTargetBounds);
}
