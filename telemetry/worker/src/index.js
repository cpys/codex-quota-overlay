...
export function validateHeartbeat(body) {
  ...
  if (!validString(body.installationId, 36, 36) || !/^[0-9a-f-]{36}$/i.test(body.installationId))
    return 'invalid installationId';
  if (!validString(body.appVersion, 5, 32) || !/^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/.test(body.appVersion))
...
async function heartbeat(request, env) {
  ...
  `
    ...
    .bind(installHash, activeDay, body.appVersion, body.platform, body.osVersion, body.locale, now, now)
    .run();
