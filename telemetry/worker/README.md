...
5. Deploy, attach the reviewed custom domain, and set `cqo.telemetryEndpoint` in the root `package.json` to the final `https://.../v1/heartbeat` URL.
...
-- Active installations by supported desktop platform
SELECT platform, COUNT(DISTINCT install_hash) AS active_installs
FROM daily_active
WHERE active_day >= date('now', '-6 days')
GROUP BY platform
ORDER BY active_installs DESC;
