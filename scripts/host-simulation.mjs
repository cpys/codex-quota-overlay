...
const cases = [
  {
    ...
    platform: /** @type {'win32'} */ ('win32'),
    window: {
      bounds: {x: 0, y: 0, width: 1280, height: 800},
      owner: {name: 'ChatGPT', path: 'C:\\Users\\test\\AppData\\Local\\OpenAI\\Codex\\ChatGPT.exe'}
    }
  },
  {
    name: 'macOS Codex x64/arm64 identity without title permission',
    platform: /** @type {'darwin'} */ ('darwin'),
    window: {
      title: '',
      bounds: {x: 0, y: 25, width: 1512, height: 920},
      owner: {name: 'Codex', bundleId: 'com.openai.codex'}
    }
  }
];

for (const testCase of cases) {
  const result = classifyTarget(testCase.window, testCase.platform);
